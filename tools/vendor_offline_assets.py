...
def vendor_dejavu() -> list[str]:
    ...
    paths.append(vendor_pdf_fonts_js())
    return paths


def vendor_pdf_fonts_js() -> str:
    """Embed DejaVu TTF as base64 for jsPDF when fetch() is blocked (file://)."""
    import base64
    ...
    if not regular_path.is_file() or not bold_path.is_file():
        raise SystemExit("DejaVu TTF files missing — run vendor_dejavu first")
    ...
    out = ROOT / "vendor" / "pdf-fonts.js"
    out.write_text(
        "/* Auto-generated by tools/vendor_offline_assets.py — DejaVu for jsPDF (file:// + offline) */\n"
        "window._LSP_PDF_FONTS = {\n"
        f"  regular: '{regular_b64}',\n"
        f"  bold: '{bold_b64}',\n"
        "};\n",
        encoding="utf-8",
    )
    print(f"  wrote {out.name} ({out.stat().st_size // 1024} KiB)")
    return "vendor/pdf-fonts.js"
