...
# Preserve the original root-relative behaviour after freezing this script under
# tools/audit/. The legacy implementation remains intentionally unchanged below.
_LEGACY_SCRIPT_PATH = Path(__file__).resolve()
_REPO_ROOT = _LEGACY_SCRIPT_PATH.parents[2]
__file__ = str(_REPO_ROOT / "audit.py")
...
if os.path.isfile(audit_wf):
    ...
    if (
        ...
        or ("tools.audit" in audit_wf_src and "--profile release" in audit_wf_src)
    ) or (
...
if os.path.isfile(audit_wf2):
    with open(audit_wf2, encoding="utf-8") as f:
        wf2 = f.read()
    if "run_ccr_differential.py" in wf2 or ("run_all_regression.py" in wf2 and "--tier release" in wf2) or ("tools.audit" in wf2 and "--profile release" in wf2):
...
if os.path.isfile(run_all_reg):
    ...
    if "native_bridge" in run_all_native_src and ("build_pages_site" in run_all_native_src or "BUILD_PAGES_SCRIPT" in run_all_native_src):
...
_audit_src57 = open(_LEGACY_SCRIPT_PATH, encoding="utf-8").read()
if (
...
if "endpointDepth = seg.fromDepth < seg.toDepth ? seg.toDepth : seg.fromDepth" in _ccr_core_src:
...
if ("python audit.py" in _apk_yml292 or "tools.audit check --profile static" in _apk_yml292) and "check_engine_parity.py" in _apk_yml292:
...
if "python audit.py" in _deploy_yml292 or "tools.audit check --profile static" in _deploy_yml292:
...
if '"test":' in open(os.path.join(_repo_root130, "package.json"), encoding="utf-8").read() and ("run_all_regression.py" in open(os.path.join(_repo_root130, "package.json"), encoding="utf-8").read() or "tools.audit run --profile release" in open(os.path.join(_repo_root130, "package.json"), encoding="utf-8").read()):
...
_ccr133 = _ccr_core_src if "function saturateLinearCCR" in _ccr_core_src else ""
