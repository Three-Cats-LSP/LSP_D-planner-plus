...
function defaultDecoCylFieldValues() {
  ...
  if (units === 'imperial') {
    return {
      size: (sizeL * CUFT_PER_LITRE).toFixed(2).replace(/0$/, '').replace(/\.0$/, ''),
      sizeMin: 0.04,
...
function appendDecoGasCardAtIdx(idx) {
  ...
  card.innerHTML = `
    ...
    <div class="gas-card-grid" style="margin-bottom:0;">
      ...
      <div class="field gas-f-num"><label>${units === 'imperial' ? 'Size (ft³)' : 'Size (L)'}</label><input type="number" id="cylDg${idx}_size" value="${cylDefaults.size}" min="${cylDefaults.sizeMin}" max="${cylDefaults.sizeMax}" step="${cylDefaults.sizeStep}" placeholder="${cylDefaults.size}"></div>
