...
function appendDecoGasCardAtIdx(idx) {
  ...
  card.innerHTML = `
    ...
    <div class="gas-card-grid" style="margin-bottom:0;">
      ...
      <div class="field gas-f-num"><label>${units === 'imperial' ? 'Size (ft³)' : 'Size (L)'}</label><input type="number" id="cylDg${idx}_size" value="${cylDefaults.size}" min="1" max="${cylDefaults.sizeMax}" step="0.5" placeholder="${cylDefaults.size}" oninput="syncVolumeInputCanonical('cylDg${idx}_size')"></div>
